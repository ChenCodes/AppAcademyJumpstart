...
def reject_odds(array)
    array = array.reject { |element| element % 2 != 0 }
end
...
def redact_five_letter_words(string)
    ...
    array = string.split(" ")
    array.each do |element|
        if element.length == 5 && element == array[-1]
            string2 += "#####"
...
def most_common_vowel(string)
    hash = {}
    list = ["a", "e", "i", "o", "u", "y"]
    list.each { |element| hash[element] = 0 }
    ...
    # hash = hash.max_by{|k,v| k}
    number = hash.max_by {|key, value| value }
    return number[0]
